...
class Net(nn.Module):
    ...
    def __init__(self, num_classes, in_channels=3, depth=5, num_fc = 2, 
        ...
        for i in range(num_fc):
            ...
            #do = nn.Dropout()
            #self.fully_connecteds.append(do)
...
def write_positives_network3(model,data_dir,data_transform,batch_size,threshold,filename,use_gpu):
...
def load_network3(state_dict = 'model_cpu_net3_state_dict.pth'):
    model = Net(num_classes=1,in_channels = 1,depth=5,num_fc=2,out_channels_first_block=16, img_size = 256)
    model.load_state_dict(torch.load(state_dict, map_location=lambda storage, loc: storage))
    return model

def train_network3(model, model_name, param_list, dataloaders, criterion, optimizer, scheduler, dataset_sizes, threshold=0.5, use_gpu=False, num_epochs=25):
    import datetime
    import time
    now = datetime.datetime.now()
    file_log = open('model3_train_log_{}.txt'.format(str(now.date())), 'w')
    file_log.write("%s\n" % model_name)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = model.state_dict()
                torch.save(best_model_wts, 'model3_state_dict_checkpoint_{}.pth'.format(str(now.date()))) #Check if it's actually in cpu readable format
    ...
    if use_gpu:
        model = model.cpu()
    torch.save(model, './model3_{}.pth'.format(str(now.date())))
    torch.save(model.state_dict(), 'model3_state_dict_final_{}.pth'.format(str(now.date())))
    return model

    
"""
def train_network3(model, model_name, param_list, dataloaders, criterion, optimizer, scheduler, dataset_sizes, use_gpu=False, num_epochs=25):
    import datetime
    import time
    now = datetime.datetime.now()
    file_log = open('model3_train_log_{}.txt'.format(str(now.date())), 'w')
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for data in dataloaders[phase]:
                ...
                # For Confusion Matrix
                running_corrects_useless += torch.sum((preds+labels.data) == 2)
                running_corrects_useful += torch.sum((preds+labels.data) == 0)
                running_total_useless += torch.sum(labels.data == 1)
                running_total_useful += torch.sum(labels.data == 0)
    ...
    if use_gpu:
        model = model.cpu()
    torch.save(model, './model3_{}.pth'.format(str(now.date())))
    torch.save(model.state_dict(), 'model3_state_dict_final{}.pth'.format(str(now.date())))
    return model
"""
